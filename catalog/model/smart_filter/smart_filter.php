...
class ModelSmartFilterSmartFilter extends Model {
	...
	public function getProductAttributes() {
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			$product_attribute_query = $this->db->query("SELECT a.attribute_id, ad.name, pa.text FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY a.sort_order, ad.name");
		...
		if(!empty($data['smart_filter']))
		{
			...
			switch ($smart_filter->name) {
				...
				case 'manufacture':
					...
					{
 						$i=0;
						foreach ($smart_filter->items as $menufacture_id) {
							//print($menufacture_id);die;
							if($i==1)
							{
	 						$sql .= " OR p.manufacturer_id = '" . (int)$menufacture_id . "'";
	 						}else{
	 						$sql .= " AND p.manufacturer_id = '" . (int)$menufacture_id . "'";
	 						}
						$i=1;			
						}
					}
					...
					{
 						$i=0;
						foreach ($smart_filter->items as $attribute_id) {
							if($i==1)
							{
	 						$sql .= " OR pa.attribute_id = '" . (int)$attribute_id . "'";
	 						}else{
	 						$sql .= " AND pa.attribute_id = '" . (int)$attribute_id . "'";
	 						}
						$i=1;			
						}
					}
